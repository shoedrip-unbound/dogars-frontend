    ...
    <form class="search" v-on:submit.prevent="search()">
      <div>
        ...
        <div style="display: contents;">
          <span>(</span><input placeholder="Species" v-model="species" /><span
            >)@</span
          >
        </div>
        <input placeholder="Item" v-model="item" />
      </div>
      <label>Level: </label><input v-model="level" /> <label>Ability: </label
      ><input v-model="ability" /> <label>- </label><input v-model="move_1" />
      <label>- </label><input v-model="move_2" /> <label>- </label
      ><input v-model="move_3" /> <label>- </label><input v-model="move_4" />
      ...
      <select v-on:focus="error = ''" v-model="format">
        ...
        <optgroup
          ...
          :label="group.value"
        >
          <option v-for="opt in group.child" :key="opt.id" :value="opt.id">{{
            opt.name
          }}</option>
        </optgroup>
      </select>
...
export default class Search extends Vue {
  ...
  search() {
    ...
    for (let o in q) {
      if (q[o] == '' || Object.keys(q[o]).length === 0) delete q[o]
    }
    delete q.groups
    if (this.random) q.random = 'true'
